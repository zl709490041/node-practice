...
superagent.get(reptileUrl).end(function(err,res){
    ...
    $('#list-container .note-list li').each(function(i,elem){
        ...
        data.push({
            ...
            collection_tag: replaceText(_this.find('.collection_tag').text()),
            ...
            likes_count: replaceText(_this.find('.ic-list-like').parent().text())
        });
    });
    //写入数据
    fs.writeFile('article.json',JSON.stringify({
        status: 0,
        data: data
    }),function(err){
        if(err) throw err;
